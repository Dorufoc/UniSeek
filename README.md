...
> **⚠️ 鸿蒙端配置提示**
>
> 当前版本为 **Debug 调试端**，测试前需手动配置后端服务器地址，**注意不能使用 `localhost`**（鸿蒙设备无法解析本机 localhost），应填写局域网内后端服务的实际 IP 地址。
>
> 在登录页面**连续点击 UniSeek 图标**即可打开**开发者菜单**，在菜单中编辑后端服务器地址，临时修改目标服务器进行测试。修改后无需重新编译，即时生效。
